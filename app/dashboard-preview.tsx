...
export function DashboardPreview() {
  ...
  return (
    <div className="relative mx-auto min-w-0 w-full max-w-2xl lg:mx-0">
      <div className="preview-window panel-demo professional-workspace system-theme" data-preview-tab={active}>
        ...
        <div className="panel-demo-app">
          <aside className="panel-demo-sidebar preview-side">
            ...
            <div className="panel-demo-nav" role="navigation" aria-label="Demonstração das áreas do painel profissional">
              {previewTabs.map((tab) => (
                <button key={tab.id} type="button" className={active === tab.id ? "system-side-nav-active" : ""} aria-label={tab.label} aria-current={active === tab.id ? "page" : undefined} onClick={() => setActive(tab.id)}>
                  <span className="panel-demo-nav-icon" aria-hidden="true">{tab.icon}</span><span className="panel-demo-nav-label">{tab.label}</span>{active === tab.id && <i className="preview-menu-click" aria-hidden="true" />}
                </button>
              ))}
            </div>
          ...
          <main className="panel-demo-main">
            <header className="panel-demo-header">
              <div><small>Bem-vindo,</small><b>Marina 👋</b></div>
              <div className="panel-demo-header-actions"><span className="panel-demo-notification" aria-hidden="true">✉<i>2</i></span><span className="panel-demo-theme" aria-hidden="true">☼</span><span className="avatar">MC</span></div>
            </header>
            <div className="panel-demo-content">
              <div className="panel-demo-breadcrumb"><b>Início</b>{active !== "inicio" && <><span>›</span><em>{activeLabel}</em></>}</div>
              <div key={active} className="preview-page">{renderPreviewPage(active)}</div>
              <div className="preview-cycle" aria-hidden="true"><span key={active} /></div>
            </div>
          </main>
        </div>
      </div>
      <div className="preview-float"><span>✓</span><div><b>Demonstração interativa</b><small>Clique no menu para explorar</small></div></div>
    </div>
  );
}
