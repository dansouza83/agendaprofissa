      ...
      <main>
        <section id="inicio" className="landing-hero relative isolate scroll-mt-32 border-b border-[#dfe9e3] px-4 py-10 sm:px-6 lg:scroll-mt-24">
          <div className="hero-depth-scene" aria-hidden="true">
            <span className="hero-line-grid hero-line-grid-main" />
          </div>
          ...
          <div className="landing-hero-grid relative mx-auto grid min-w-0 max-w-7xl items-center gap-10 lg:grid-cols-[.92fr_1.08fr]">
            ...
            <div data-reveal="right" data-reveal-delay="1"><DashboardPreview /></div>
          </div>
        </section>
        ...
        <section id="recursos" className="scroll-mt-32 px-4 py-20 sm:px-6 lg:scroll-mt-24 lg:py-28">
          <div className="mx-auto max-w-7xl">
            <SectionIntro reveal eyebrow="Tudo no lugar" title="Menos tempo organizando. Mais tempo atendendo." text="Recursos essenciais para uma rotina profissional, sem excesso de telas ou complicação." />
            <div className="resource-grid mt-10">
              {resources.map((item, index) => <article key={item.title} className="feature-card" data-reveal={index % 3 === 0 ? "left" : index % 3 === 2 ? "right" : "up"} data-reveal-delay={String(index % 3)}><div className="feature-card-heading"><span className="feature-icon">{item.icon}</span><h3>{item.title}</h3></div><p>{item.text}</p></article>)}
            </div>
            <article className="whatsapp-spotlight mt-8" data-reveal="up" data-reveal-delay="1">
              <div className="whatsapp-spotlight-copy">
                <span className="whatsapp-kicker"><b aria-hidden="true">WA</b> Mensagens pelo WhatsApp</span>
                <h3>Atualize o cliente sem copiar, colar ou redigitar tudo.</h3>
                <p>O Agenda Profissa prepara uma mensagem com nome, serviço, data, horário e situação do pagamento. Você revisa e abre a conversa certa no WhatsApp em poucos toques.</p>
                <ul aria-label="Benefícios das mensagens pelo WhatsApp">
                  <li><span>✓</span> Menos erros nos horários</li>
                  <li><span>✓</span> Confirmações mais rápidas</li>
                  <li><span>✓</span> Atendimento próximo e profissional</li>
                </ul>
                <Link href="/sistema?cadastro=profissional" className="whatsapp-cta">Quero facilitar meu atendimento <span aria-hidden="true">→</span></Link>
                <small>Você mantém o controle: a mensagem só é enviada depois da sua confirmação no WhatsApp.</small>
              </div>
              <div className="whatsapp-demo" aria-label="Exemplo de confirmação preparada para WhatsApp">
                <div className="whatsapp-demo-top"><span>Mensagem pronta</span><b>WhatsApp</b></div>
                <div className="whatsapp-demo-chat">
                  <p>Olá, Ana! Seu pagamento foi confirmado e o horário para Limpeza de pele, em 28/08 às 14:30, está reservado.</p>
                  <time>Agora</time>
                </div>
                <div className="whatsapp-demo-action"><span>Revise a mensagem</span><b>Abrir WhatsApp →</b></div>
              </div>
            </article>
          </div>
        </section>
        ...
        <section id="como-funciona" className="scroll-mt-32 bg-[#183f37] px-4 py-20 text-white sm:px-6 lg:scroll-mt-24 lg:py-28">
          <div className="mx-auto max-w-7xl">
            ...
            <div className="steps-grid mt-10">
              ...
              ].map(([number,title,text], index) => <article key={number} className="step-card" data-reveal={index === 0 ? "left" : index === 2 ? "right" : "up"} data-reveal-delay={String(index)}><div className="step-card-heading"><span className="step-number">{number}</span><h3>{title}</h3></div><p>{text}</p></article>)}
            </div>
          </div>
        </section>
        ...
        <section id="para-quem" className="scroll-mt-32 px-4 py-20 sm:px-6 lg:scroll-mt-24 lg:py-28">
          <div className="audience-grid mx-auto max-w-7xl">
            <article className="audience-card audience-professional" data-reveal="left">
            ...
            <article className="audience-card audience-client" data-reveal="right" data-reveal-delay="1">
        ...
        <section id="planos" className="landing-pricing scroll-mt-32 border-y border-[#dfe9e3] bg-white px-4 py-20 sm:px-6 lg:scroll-mt-24 lg:py-28">
          <div className="mx-auto max-w-7xl">
            <SectionIntro reveal centered eyebrow="Planos para profissionais" title="Invista na organização que acompanha o seu negócio todos os dias." text="Os dois planos liberam o painel profissional completo. Escolha a flexibilidade do pagamento mensal ou a economia e a previsibilidade do plano anual." />
            <div data-reveal="up" data-reveal-delay="1"><PublicPricing /></div>
            <div className="pricing-assurance mx-auto mt-8 max-w-5xl" data-reveal="up" data-reveal-delay="2">
        ...
        <section id="seguranca" className="landing-security scroll-mt-32 border-y border-[#dfe9e3] bg-white px-4 py-20 sm:px-6 lg:scroll-mt-24 lg:py-24">
          <div className="mx-auto grid max-w-7xl items-center gap-12 lg:grid-cols-[.8fr_1.2fr]">
            <div className="security-mark" aria-hidden="true" data-reveal="left"><span>✓</span><div><b>Privacidade desde a base</b><small>Arquitetura multitenant</small></div></div>
            <div data-reveal="right" data-reveal-delay="1"><p className="section-eyebrow">Segurança e privacidade</p><h2 className="section-title max-w-3xl">Cada negócio no seu espaço. Cada pessoa no seu nível de acesso.</h2><p className="mt-5 max-w-2xl text-lg leading-8 text-[#63736e]">O banco de dados aplica regras por negócio em cada leitura e alteração. A autenticação, a recuperação de senha e o armazenamento de sessão foram planejados para a publicação online.</p><div className="mt-7 flex flex-wrap gap-3"><Link className="text-link-chip" href="/seguranca">Conhecer as medidas</Link></div></div>
          </div>
        </section>
        ...
        <section className="landing-final-cta px-4 pb-20 sm:px-6 lg:pb-28">
          <div className="landing-cta relative mx-auto max-w-7xl overflow-hidden rounded-[2rem] bg-[#e9a878] px-6 py-14 text-center text-[#18362f] sm:px-10 lg:py-20" data-reveal="up">
...
function SectionIntro({ eyebrow, title, text, centered = false, light = false, reveal = false }: { eyebrow: string; title: string; text: string; centered?: boolean; light?: boolean; reveal?: boolean }) {
  return <div className={centered ? "mx-auto max-w-3xl text-center" : "max-w-3xl"} {...(reveal ? { "data-reveal": "up" } : {})}><p className={`section-eyebrow ${light ? "!text-[#9dd4c4]" : ""}`}>{eyebrow}</p><h2 className={`section-title ${light ? "!text-white" : ""}`}>{title}</h2><p className={`mt-5 text-lg leading-8 ${light ? "text-[#c4dad3]" : "text-[#64756f]"}`}>{text}</p></div>;
}
