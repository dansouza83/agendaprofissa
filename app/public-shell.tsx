...
const mainNavigation = [
  ...
  ["Para quem", "/#para-quem"], ["Segurança", "/#seguranca"], ["FAQ", "/#faq"],
] as const;
...
export function PublicFooter() {
  return <footer className="public-footer bg-[#122f29] px-4 py-14 text-white sm:px-6"><div className="mx-auto max-w-7xl"><div className="grid gap-10 border-b border-white/10 pb-10 md:grid-cols-[1.2fr_.8fr_.8fr]"><div><Logo inverse/><p className="mt-5 max-w-sm text-sm leading-6 text-[#abc5bd]">Gestão de agendamentos para profissionais e negócios que cuidam de pessoas.</p></div><div><h2 className="text-sm font-extrabold">Produto</h2><div className="mt-4 grid gap-3 text-sm text-[#abc5bd]"><Link href="/#recursos">Recursos</Link><Link href="/#como-funciona">Como funciona</Link><Link href="/faq">Perguntas frequentes</Link><Link href="/sistema">Acessar sistema</Link></div></div><div><h2 className="text-sm font-extrabold">Legal e confiança</h2><div className="mt-4 grid gap-3 text-sm text-[#abc5bd]">{legalLinks.slice(1).map(([label,href])=><Link key={href} href={href}>{label}</Link>)}</div></div></div><div className="flex flex-col gap-3 pt-7 text-xs text-[#8faea5] sm:flex-row sm:items-center sm:justify-between"><p>© {new Date().getFullYear()} Agenda Profissa. Ambiente de MVP.</p><p>Pagamentos não disponíveis nesta versão.</p></div></div></footer>;
}
