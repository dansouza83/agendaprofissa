...
export function Logo({ inverse = false }: { inverse?: boolean }) {
  return <Link href="/" aria-label="Agenda Profissa — página inicial" className="brand-lockup flex min-w-0 items-center gap-2.5 sm:gap-3"><span className={`brand-symbol-tile grid h-10 w-10 flex-none place-items-center overflow-hidden rounded-xl p-1 shadow-sm ${inverse ? "brand-symbol-inverse bg-transparent shadow-none" : "bg-[#eaf7f2]"}`}><img src="/brand/agenda-profissa-symbol-v2.png" alt="" aria-hidden="true" className={`h-full w-full object-contain ${inverse ? "brand-symbol-white" : "brand-symbol-adaptive"}`} /></span><span className="brand-lockup-copy min-w-0"><b className={`brand-wordmark block text-lg leading-none ${inverse ? "text-white" : "text-[#173f37]"}`}><span>Agenda</span> <span className="brand-wordmark-accent">Profissa</span></b><small className={`hidden sm:block ${inverse ? "text-[#b7d5ca]" : "text-[#52675f]"}`}>Seu negócio organizado</small></span></Link>;
}
...
export function PublicHeader() {
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      if (headerScrollFrame.current === null) headerScrollFrame.current = window.requestAnimationFrame(syncHeader);
    };
    ...
    return () => {
      ...
      if (headerScrollFrame.current !== null) window.cancelAnimationFrame(headerScrollFrame.current);
    };
  }, []);
  ...
  const scrollToSection = (event: ReactMouseEvent<HTMLAnchorElement>, href: string) => {
    ...
    const headerOffset = (document.querySelector<HTMLElement>(".public-header")?.offsetHeight ?? (window.innerWidth < 1024 ? 116 : 72)) + 12;
  ...
  return <header className={`public-header ${compact ? "public-header-compact" : ""} fixed inset-x-0 top-0 z-50 border-b border-[#dfe8e3] bg-[#f8faf7]/95 px-4 backdrop-blur-xl sm:px-6`}><div className="public-header-main mx-auto flex max-w-7xl items-center justify-between"><div className="public-header-brand"><Logo/></div><nav aria-label="Navegação principal" className="public-desktop-nav hidden items-center gap-5 xl:flex">{mainNavigation.map(([label,href])=><Link key={href} href={href} onClick={(event) => scrollToSection(event, href)} className="text-sm font-bold text-[#526660] transition hover:text-[#2f7d70]">{label}</Link>)}</nav><div className="public-header-actions flex items-center gap-1 sm:gap-2"><ThemeToggle/><Link href="/sistema" className="inline-flex min-h-10 items-center rounded-xl px-2 text-xs font-extrabold text-[#2f7d70] sm:min-h-11 sm:px-4 sm:text-sm">Entrar</Link><Link href="/sistema?cadastro=profissional" className="public-create-account inline-flex min-h-10 items-center rounded-xl bg-[#2f7d70] px-2 text-xs font-extrabold text-white shadow-[0_8px_20px_#2f7d7026] hover:bg-[#276b61] sm:min-h-11 sm:px-4 sm:text-sm">Criar conta</Link></div></div><nav aria-label="Atalhos da página" className="public-mobile-nav -mx-4 flex h-11 items-center gap-1 overflow-x-auto border-t border-[#dfe8e3] px-4 xl:hidden sm:-mx-6 sm:px-6">{mainNavigation.map(([label,href])=><Link key={href} href={href} onClick={(event) => scrollToSection(event, href)} className="flex min-h-8 flex-none items-center rounded-full px-3 text-xs font-extrabold text-[#526660] hover:bg-[#e9f3ef] hover:text-[#2f7d70]">{label}</Link>)}</nav></header>;
}
